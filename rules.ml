let rules =
  let normal = [
    ...
    ("(2/3) Throwing the dice", [
        ...
        "are white. If a white corner is pointing upwards, then it counts as a point.";
        "The total number of points is how much the player can move this turn.";
        "If the total is zero, the player's turn is skipped."
      ]);
    ("(3/3) Movement", [
        "You can either place a reserve pawn on the board if it lands on an empty cell";
        "or move a present pawn if it lands on an empty cell or ennemy pawn.";
        "In this last case, the enemy pawn is placed back into his reserve.";
        "A pawn can exit the board if it lands exactly on the exit, which increases the score.";
        "If a pawn lands on a special cell, then the player can immediately throw";
